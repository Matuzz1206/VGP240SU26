...
//lets is the smaller x position, right is the highest x position
//this allow us to always index in a positive direction (simple math)
...
void Rasterizer::DrawFilledTriangle(const Vertex& a, const Vertex& b, const Vertex& c)
{
	...
	//if a and b are the same, trianglw is a flat top
	...
	{
		int startY = static_cast<int>(a.pos.y);
		int endY = static_cast<int>(b.pos.y);
		for (int y = startY; y <= endY; ++y)
		{
			float t = static_cast<float>(y - startY) / dy;
